...
# Based on the avg. Edge Weights of Nodes
...
pdfAvg0 <- pdf(paste('~/Desktop/NetVis/networks',my.site, my.node, 'Avg_simpl.pdf', sep="_")) 
...
for (file in files_sorted){
    ...
    for (i in 1:nrow(files_prev)) {
      if (as.numeric(files_prev[i, 'Mean.Prevalence']) > 0.03) {
        AvgEW_List <- c(AvgEW_List, files_prev[i, 'Topic..Num.'])
      } 
    ...
    Nodes_to_keep <- c()
    for (i in NodesList) {
      if (as.numeric(i) %in% AvgEW_List || as.numeric(i) == as.numeric(my.node)) {
