#Created to Fix a bug with the Atlantic (52-Vaccines) Files, Minor changes from LocalNetPop file
...
for (file in files_sorted){
  try ({
    ...
    for (i in 1:nrow(nodes)) {
      for (j in 1:length(x)){ # Debugging (Replace x)
    ...
    for (j in 1:nrow(y)) {
      ...
      #Debug, Making sure lists are of equal length
      Name_Find <- as.character(baseName[j])
      if (!(baseNum[j] %in% NodeNum)) {
        NodeNum[[Name_Find]] <- (baseNum[j])
...
#Debug, Making sure lists are of equal length
NodeNum <- c(NodeNum[1:157], names(continuous_values[157]), NodeNum[158:length(NodeNum)])
dev.off()
