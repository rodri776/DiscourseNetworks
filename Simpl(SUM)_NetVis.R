...
# Based on the Edge Weights Sums of Nodes
...
#Standardize Threshold (length of dataset*.01^2 {ex. Atlantic dataset is 70m long, thus it is 70*0.01=0.7^2=0.49})
if (my.site == 'atlantic') {
  threshold <- 0.49
...
pdfSum1 <- pdf(paste('~/Desktop/NetVis/networks',my.site, my.node, 'Sum_simpl.pdf', sep="_")) 
...
for (file in files_sorted){
  ...
  for (i in 1:nrow(files_prev)) {
    if (as.numeric(files_prev[i, 'Continuous..edge.weight.']) > as.numeric(threshold)) {
      SumEW_List <- c(SumEW_List, files_prev[i, 'Topic..Num.'])
    } 
  }
  #Filter Further
  Nodes_to_keep <- c()
  for (i in NodesList) {
    if (as.numeric(i) %in% SumEW_List || as.numeric(i) == as.numeric(my.node)) {
